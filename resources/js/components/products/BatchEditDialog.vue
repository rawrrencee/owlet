...
const commonSubcategoryId = computed((): number | null => {
    ...
    if (first === undefined) return null;
    return props.products.every((p) => p.subcategory_id === first)
        ? first
        : null;
});
    ...
    >
        <form @submit.prevent="handleSubmit" class="flex flex-col gap-4 p-4">
            ...
            <Message severity="info" :closable="false" class="!m-0">
                Editing {{ products.length }} product(s). Only checked fields
                will be updated.
            </Message>
            ...
            <div class="rounded-lg border border-border p-4">
                ...
                <div class="flex flex-col gap-3">
                    ...
                    <div class="flex items-center gap-3">
                        <Checkbox
                            v-model="form.apply_brand"
                            :binary="true"
                            input-id="apply_brand"
                        />
                        <label for="apply_brand" class="w-24 text-sm"
                            >Brand</label
                        >
                        <Select
                    ...
                    <div class="flex items-start gap-3">
                        <Checkbox
                            v-model="form.apply_category"
                            :binary="true"
                            input-id="apply_category"
                            class="mt-2"
                        />
                        <label for="apply_category" class="mt-2 w-24 text-sm"
                            >Category</label
                        >
                    ...
                    <div class="flex items-center gap-3">
                        <Checkbox
                            v-model="form.apply_supplier"
                            :binary="true"
                            input-id="apply_supplier"
                        />
                        <label for="apply_supplier" class="w-24 text-sm"
                            >Supplier</label
                        >
                        <Select
            ...
            <div class="rounded-lg border border-border p-4">
                ...
                <div class="flex items-center gap-3">
                    <Checkbox
                        ...
                        :binary="true"
                        input-id="apply_status"
                    />
                    <label for="apply_status" class="w-24 text-sm"
                        >Active</label
                    >
                    <div class="flex items-center gap-2">
                        <ToggleSwitch
                            v-model="form.is_active"
                            :disabled="!form.apply_status"
                        />
                        <span class="text-sm text-muted-foreground">
                            {{ form.is_active ? 'Active' : 'Inactive' }}
                            <template v-if="isMixed(commonIsActive)">
                                (Mixed)</template
                            >
                        </span>
                    </div>
            ...
            <div class="rounded-lg border border-border p-4">
                ...
                <div class="flex flex-col gap-3">
                    <div class="flex items-center gap-3">
                        <Checkbox
                            v-model="form.apply_prices"
                            :binary="true"
                            input-id="apply_prices"
                        />
                        <label for="apply_prices" class="w-24 text-sm"
                            >Mode</label
                        >
                        <SelectButton
                    ...
                    <div
                        v-if="form.apply_prices"
                        class="mt-2 flex flex-col gap-2"
                    >
                        <div
                            v-for="(
                                adjustment, index
                            ) in form.price_adjustments"
                        ...
                        >
                            ...
                            <div>
                                ...
                                <InputNumber
                                    v-model="
                                        form.price_adjustments[index].cost_price
                                    "
                                    :min="
                            ...
                            <div>
                                ...
                                <InputNumber
                                    v-model="
                                        form.price_adjustments[index].unit_price
                                    "
                                    :min="
                                        form.price_mode === 'fixed'
                                            ? 0
                    ...
                    <Message
                        ...
                        class="!mt-2"
                    >
                        Store-specific prices cannot be batch edited. Only base
