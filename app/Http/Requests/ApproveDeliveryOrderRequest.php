...
class ApproveDeliveryOrderRequest extends FormRequest
{
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            foreach ($items as $index => $item) {
                ...
                $originalItem = \App\Models\DeliveryOrderItem::find($item['id'] ?? null);
                if ($originalItem && isset($item['received_quantity'])) {
                    if ((int) $item['received_quantity'] !== $originalItem->quantity && empty($item['correction_note'])) {
                        $validator->errors()->add(
                            ...
                            'A correction note is required when the received quantity differs from the requested quantity.'
                        );
                    }
                }
            }
